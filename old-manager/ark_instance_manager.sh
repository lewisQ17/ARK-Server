...
install_base_server() {
    ...
    "$STEAMCMD_DIR/steamcmd.sh" +force_install_dir "$SERVER_FILES_DIR" +login anonymous +@sSteamCmdForcePlatformType windows +app_update 2430930 validate +quit
...
start_server() {
    ...
    if ! grep -qw avx /proc/cpuinfo || ! grep -qw avx2 /proc/cpuinfo; then
        echo -e "${RED}Missing CPU flags AVX/AVX2 in this VM. Enable CPU passthrough/host model in hypervisor before starting ASA.${RESET}"
        return 1
    fi
...
change_map() {
    ...
    echo -e "${CYAN}Checking/downloading missing map assets via SteamCMD...${RESET}"
    "$STEAMCMD_DIR/steamcmd.sh" +force_install_dir "$SERVER_FILES_DIR" +login anonymous +@sSteamCmdForcePlatformType windows +app_update 2430930 validate +quit || \
        echo -e "${YELLOW}SteamCMD update returned non-zero. If map start fails, run base update manually.${RESET}"
